...
import { editarEventoAction } from '@/actions/admin-actions' // Confirma se o caminho está certo
...
export default function ModalEditarEvento({ evento }: { evento: any }) {
    ...
    async function handleAction(formData: FormData) {
        ...
        if (res.ok) {
            setIsOpen(false) // Fecha o modal se o update correr bem
        } else {
    ...
    return (
        <>
            {/* O BOTÃO PEQUENO DE EDITAR (Fica ao lado do título do evento) */}
            <button
                type="button"
                onClick={() => setIsOpen(true)}
                className="p-2 bg-bg2 border border-soft text-muted hover:text-figueira hover:border-figueira hover:bg-figueira/5 rounded-xl transition-all active:scale-90 shadow-sm flex items-center justify-center"
                title="Editar Culto/Evento"
            >
                <Edit3 size={16} />
            </button>
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-bg w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden border border-soft flex flex-col animate-in zoom-in-95 duration-300">
                        ...
                        <form action={handleAction} className="p-8 space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-muted tracking-[0.2em] ml-2 flex items-center gap-2">
                                    <Type size={12} /> Descrição / Tema (Opcional)
                                </label>
                                <textarea
                                    name="descricao"
                                    defaultValue={evento.descricao || ''}
                                    rows={3}
                                    className="w-full bg-bg2 border-2 border-soft rounded-2xl px-5 py-4 text-xs font-black text-fg focus:border-figueira outline-none transition-colors resize-none"
                                    placeholder="Ex: Culto de Jovens - Tema: Fé"
                                />
                            </div>
