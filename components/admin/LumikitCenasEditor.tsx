...
import { salvarLumikitCenas, type LumikitScene, type LumikitConfig } from '@/actions/midia-actions'
...
const ICONES: { id: string; icon: LucideIcon; nome: string }[] = [
    ...
    { id: 'moon', icon: Moon, nome: 'Lua' },
    { id: 'sparkles', icon: Sparkles, nome: 'Brilho' },
    ...
    { id: 'eye', icon: Eye, nome: 'Olho' },
    { id: 'flame', icon: Flame, nome: 'Chama' },
    { id: 'lightning', icon: CloudLightning, nome: 'Trovao' },
    { id: 'sunrise', icon: Sunrise, nome: 'Nascer' },
    { id: 'mic', icon: Mic, nome: 'Microfone' },
    { id: 'church', icon: Church, nome: 'Igreja' },
]

export function getIconComponent(iconId: string): LucideIcon {
    return ICONES.find(i => i.id === iconId)?.icon || Lightbulb
}
...
export default function LumikitCenasEditor({ initialConfig }: Props) {
    ...
    const update = (id: string, field: keyof LumikitScene, value: any) => {
    ...
    const handleSave = async () => {
        setSaving(true)
        setMsg('')
        const res = await salvarLumikitCenas({ scenes })
        setSaving(false)
        setMsg(res.ok ? 'Botoes guardados!' : (res.error || 'Erro'))
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-bg2 border border-soft rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-soft">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-xs font-black uppercase tracking-widest text-fg">Botoes de Iluminacao</h3>
                ...
                <div className="p-5 space-y-4">
                    {scenes.length === 0 && (
                        <p className="text-[10px] text-muted text-center py-4 font-bold uppercase tracking-widest">
                            Nenhum botao configurado. Clique em "Adicionar" para criar.
                        </p>
                    )}

                    {scenes.map((scene) => {
                        const IconPreview = ICONES.find(i => i.id === scene.icone)?.icon || Lightbulb
                        return (
                            <div key={scene.id} className="p-4 bg-bg border border-soft rounded-xl space-y-4">
                                {/* Linha 1: Nome + Tipo + Apagar */}
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <input type="text" value={scene.nome} onChange={e => update(scene.id, 'nome', e.target.value)}
                                            placeholder="Ex: Oracao, Louvor, Flash..."
            ...
            <div className="bg-bg2 border border-soft rounded-2xl p-4 text-[9px] text-muted/60 space-y-1">
                ...
                <p><strong>Push:</strong> Clica e executa o script uma vez (ex: flash, transicao).</p>
                <p><strong>Toggle:</strong> Liga/desliga — usa 2 scripts diferentes (ex: ligar cena / desligar cena).</p>
                <p><strong>Script ID:</strong> No Holyrics, va a Ferramentas &gt; Scripts e copie o ID do script que controla o Lumikit.</p>
                <p>Os comandos sao enviados via API do Holyrics que ja esta conectado.</p>
            </div>
