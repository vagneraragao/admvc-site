...
export default function MeuPerfilClient({ membro, escolaridades = [] }: any) {
    ...
    // Função auxiliar para os cartões de Documentos
        ...
        <main className="max-w-5xl mx-auto p-4 sm:p-6 pb-20 animate-in fade-in duration-700">
            <form action={handleAction} className="space-y-8">
                ...
                <div className="bg-bg2 border border-soft rounded-[3rem] p-8 md:p-12 shadow-sm min-h-[500px]">
                    ...
                    <div className={abaAtiva === 'pessoal' ? 'block space-y-8 animate-in fade-in slide-in-from-bottom-4' : 'hidden'}>
                        <div className="flex flex-col md:flex-row items-center gap-8 pb-8 border-b border-soft/50">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted font-bold uppercase tracking-widest mt-1">Informações básicas de contacto.</p>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 pt-8 border-t border-soft">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-muted flex items-center gap-2 mb-6">
                                <Lock size={14} className="text-figueira" /> Segurança da Conta
                            </h4>
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <Input
                    ...
                    <div className={abaAtiva === 'eclesiastico' ? 'block animate-in fade-in slide-in-from-bottom-4' : 'hidden'}>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="md:col-span-2 lg:col-span-3 space-y-2 mt-2">
                                <label className="text-[10px] font-black uppercase text-muted ml-4 tracking-widest">Testemunho / Notas Pessoais</label>
                                <textarea
                                    name="notes"
                                    rows={4}
                    ...
                    {/* ========================================================= */}
                    {/* ABA 5: ADMINISTRATIVO (APENAS LEITURA PARA O MEMBRO)      */}
                    {/* ========================================================= */}
                    <div className={abaAtiva === 'administrativo' ? 'block animate-in fade-in slide-in-from-bottom-4' : 'hidden'}>
                        <div className="bg-orange-500/5 border border-orange-500/20 rounded-3xl p-6 mb-8 flex items-start gap-4">
                            <AlertCircle size={20} className="text-orange-500 shrink-0 mt-0.5" />
                            <div>
                                <h4 className="text-sm font-black uppercase italic tracking-tighter text-orange-700">Painel de Acesso Restrito</h4>
                                <p className="text-[10px] text-orange-600/80 font-bold uppercase tracking-widest mt-1">
                                    Estes dados são geridos exclusivamente pela administração da igreja. Estão aqui apenas para sua consulta.
                                </p>
                            </div>
                        </div>

                        <div className="grid md:grid-cols-3 gap-6 opacity-80 pointer-events-none">
                            <Input
                                label="Acesso ao Sistema Permitido?"
                                value={membro.is_active ? "Sim (Conta Ativa)" : "Não (Conta Bloqueada)"}
                                readOnly
                                className="bg-soft/50 border-dashed"
                            />
                            <Input
                                label="Nível de Permissão (Role)"
                                value={membro.role || "USER"}
                                readOnly
                                className="bg-soft/50 border-dashed font-black text-figueira"
                            />
                            <Input
                                label="Identificador Loyverse (Cantina)"
                                value={membro.loyverse_id || "Não Integrado"}
                                readOnly
                                className="bg-soft/50 border-dashed"
                            />
                        </div>
