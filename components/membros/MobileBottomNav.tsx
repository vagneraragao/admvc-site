...
import {
    Home, Calendar, Music2, MonitorPlay, HeartHandshake, Store,
...
export default function MobileBottomNav({ permissoes, proximosEventos = [] }: Props) {
    ...
    // Build dynamic tabs
    const tabs: { key: string; icon: typeof Home; label: string; href?: string; action?: () => void }[] = [
        { key: 'home', icon: Home, label: 'Inicio', href: '/membros/dashboard' },
        { key: 'agenda', icon: Calendar, label: 'Agenda', action: () => setAgendaAberta(true) },
    ]

    if (permissoes.isLouvor) tabs.push({ key: 'louvor', icon: Music2, label: 'Louvor', href: '/midia/mesax32' })
