        ...
        <main className="max-w-screen-2xl mx-auto p-4 sm:p-6 pb-20 animate-in fade-in duration-700 h-screen flex flex-col">
            ...
            <header className="mb-4 flex flex-col xl:flex-row justify-between items-center gap-4 bg-bg2 border border-soft p-3 rounded-2xl shadow-sm shrink-0">
                <div className="flex items-center gap-3 w-full xl:w-auto shrink-0 px-2">
                    <div className={`p-2 rounded-lg shadow-md transition-colors ${status === 'connected' ? 'bg-green-500 text-white' : 'bg-figueira text-white'}`}>
                        <MonitorPlay size={18} />
                    </div>
                    <div>
                        <h1 className="text-lg font-black italic uppercase tracking-tighter text-fg leading-none">Holyrics Live</h1>
                        <span className="text-[9px] font-black uppercase tracking-widest text-muted flex items-center gap-1 mt-1">
                            {status === 'connected' ? <CheckCircle2 size={10} className="text-green-500" /> : <Wifi size={10} />}
                            {status === 'connected' ? 'Sincronizado' : 'Aguardando Conexão'}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2 w-full xl:w-auto">
                    <input
                        ...
                        className={`bg-bg border border-soft rounded-lg px-3 py-2 text-xs font-bold text-fg outline-none w-full sm:w-44 transition-all ${status === 'connected' ? 'opacity-50 cursor-not-allowed bg-soft/30' : 'focus:border-figueira'}`}
                    />

                    <div className="relative w-full sm:w-36">
                        <Key size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" />
                        <input
                            type="text"
                            value={apiToken}
                            ...
                            disabled={status === 'connected'}
                            placeholder="Token"
                            className={`w-full bg-bg border border-soft rounded-lg pl-8 pr-3 py-2 text-xs font-bold text-fg outline-none transition-all ${status === 'connected' ? 'opacity-50 cursor-not-allowed bg-soft/30' : 'focus:border-figueira'}`}
                        />
                    </div>

                    {status === 'connected' ? (
                        <button onClick={handleDisconnect} className="w-full sm:w-auto bg-red-500/10 text-red-500 border border-red-500/20 px-4 py-2 rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-red-500 hover:text-white transition-all flex justify-center items-center gap-2 active:scale-95">
                            <XCircle size={14} /> Sair
                        </button>
                    ) : (
                        <button onClick={testConnection} disabled={status === 'testing'} className="w-full sm:w-auto bg-fg text-bg px-4 py-2 rounded-lg font-black uppercase text-[10px] tracking-widest hover:bg-figueira transition-all flex justify-center items-center gap-2 active:scale-95 disabled:opacity-50">
